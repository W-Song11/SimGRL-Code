...
def _get_out_shape_feat_encoder(in_shape, layers, layers2):
    x = torch.randn(*in_shape).unsqueeze(0)
    if len(x.shape)==4:
        B,C,H,W = x.shape
        S = 3
...
class SharedCNN_feat_stack(nn.Module):
    def __init__(self, obs_shape, num_layers=11, num_filters=32):
        super().__init__()
        assert len(obs_shape) == 3
        self.num_layers = num_layers
        ...
        self.layers = [CenterCrop(size=84), NormalizeImg(), nn.Conv2d(3, num_filters1, 3, stride=2)]
        for _ in range(1, num_layer1):
            self.layers.append(nn.ReLU())
            self.layers.append(nn.Conv2d(num_filters1, num_filters1, 3, stride=1))
        self.layers2 = []
        self.layers2.append(nn.ReLU())
        self.layers2.append(nn.Conv2d(num_filters1*3, num_filters, 3, stride=1))
        for _ in range(num_layer1+1, num_layers):
            self.layers2.append(nn.ReLU())
            self.layers2.append(nn.Conv2d(num_filters, num_filters, 3, stride=1))
        self.layers = nn.Sequential(*self.layers)
        self.layers2 = nn.Sequential(*self.layers2)
        self.out_shape = _get_out_shape_feat_encoder(obs_shape, self.layers, self.layers2)
        self.apply(weight_init)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, detach=False, select_w=False):
        if select_w:
            x, w = self.shared_cnn(x, select_w)
            x = self.head_cnn(x)
            if detach:
                x = x.detach()
...
class Critic(nn.Module):
    ...
    def forward(self, x, action, detach=False, select_w=False):
        if select_w:
            x, w = self.encoder(x, detach, select_w)
            return self.Q1(x, action), self.Q2(x, action), w
        else:
            x = self.encoder(x, detach)
